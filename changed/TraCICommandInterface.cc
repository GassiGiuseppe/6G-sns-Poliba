...
// MODIFIED BY GASSI GIUSEPPE, POLIBA
void TraCICommandInterface::setOrder(uint32_t order)
{
    EV_INFO << "Set SUMO Order..." << endl;
    const uint8_t CMD_SETORDER = 3;
    TraCIBuffer orderbuf = TraCIBuffer();
    connection.query(CMD_SETORDER, orderbuf << order);
    // ASSERT(orderbuf.eof());
    EV_INFO << "Set Order done!" << endl;
}
